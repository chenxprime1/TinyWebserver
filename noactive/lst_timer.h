...
public:
    ...
    void tick() {
        ...
        while (tmp) {
            ...
            head = tmp->next;
            if (head) { // 如果 head 不为空 那么将 head 的前一个结点置为NULL head 为空就不用执行这一步了
                head->prev = NULL;
            }
            delete tmp;
            tmp = head;
...
private:
    ...
    void add_timer(util_timer* timer, util_timer* lst_head) {
        ...
        并将目标定时器插入该节点之前 */
        while (tmp) {
            if (timer->expire < tmp->expire) { // timer 的数值比 tmp 的数值小 将 timer 插入到 prev 和 tmp 之间
                prev->next = timer;
                timer->next = tmp;
                tmp->prev = timer;
                timer->prev = prev;
            }
            prev = tmp; // 否则 prev 和 tmp 后移
            tmp = tmp->next;
        }
