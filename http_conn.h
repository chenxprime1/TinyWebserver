...
public:
    ...
    /* 解析客户端请求时，主状态机的状态，
    CHECK_STATE_REQUSETLINE: 当前正在分析请求行
    ...
    */
    ...
    /*
        ...
        FILE_REQUEST            :   文件请求，取文件成功
        INTERNAL_ERROR          :   表示服务器内部错误
        CLOSED_CONNECTION       :   表示客户端已经关闭连接了
    */
    enum HTTP_CODE { NO_REQUEST = 0, GET_REQUEST, BAD_REQUEST, NO_RESOURSE, FORBIDDENT_REQUEST, FILE_REQUEST, INTERNAL_ERROR, CLOSED_CONNECTION };
public:
    http_conn() {};
    ~http_conn() {};
public:
...
private:
    ...
    struct iovec m_iv[2];                   // 我们将采用writev来执行写操作，所以定义下面两个成员，其中m_iv_count表示被写内存块的数量。 这里定义长度为2的数组 分别对应这两块内存 m_write_buf 和 m_file_address 在process_write中进行初始化这两块内存
    int m_iv_count;
    ...
    struct iovec
    {
        void *iov_base;	// Pointer to data.  内存的起始位置
        size_t iov_len;	// Length of data.  内存的长度
    };
    */
